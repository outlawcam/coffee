  ...
  return (
    <section id="craft" className="sect" style={{ background: 'var(--dark-900)', color: 'var(--paper-100)', padding: '96px 40px', scrollMarginTop: 72 }}>
      <div className="grid-split" style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
        {/* Image */}
        <figure className="halftone" style={{ margin: 0, position: 'relative', width: '100%', aspectRatio: '4 / 5', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.18)' }}>
          <image-slot id="land-craft" shape="rect" fit="cover" src="/assets/craft.jpg" placeholder="Roastery / makers photo"></image-slot>
          <figcaption style={{ position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 3, fontFamily: 'var(--font-wire)', fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--paper-100)', background: 'rgba(32,27,22,0.82)', padding: '7px 12px' }}>Fig. 2 — Cupping the day's lots.</figcaption>
        </figure>
        {/* Text */}
        <div>
          ...
          <h2 className="h2" style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 40, letterSpacing: '-0.02em', color: 'var(--ink-fill-rev)', WebkitTextStroke: '0.8px var(--ink-edge-rev)', margin: '0 0 20px', lineHeight: 1.06 }}>We roast to protect the cup, not to chase a color.</h2>
